...
class FusionCharts
    ...
    def __init__(self type id width height renderAt dataFormat dataSource) 
        ...
        if isinstance(dataSource TimeSeries)
            ...
            selfconstructorOptions['dataSource'] = "__TS__"
        else:
    ...
    def render(self):
        ...
        if isinstance(self timeSeriesObject TimeSeries)
            self readyJson = self readyJson replace("__TS__"  self timeSeriesObjectGetDataStore())
        else:
            self readyJson = selfreadyJsonreplace("__TS__" "")
...
class TimeSeries:
    ...
    def GetDataSource(self)
        ...
        for dic in self attributes
            for key in dic
                stringBuilderAppendLine("{0} {1} " format(key dic[key]))
...
class FusionTable:
    ...
    def CreateFilter(self filterType columnName *values):
        ...
        return switcher get(filterType "FusionCharts DataStore Operators {0}('{1}' {2})" format(fx columnName my list[0]))
